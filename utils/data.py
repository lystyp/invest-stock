...
class Data():
    ...
    def __init__(self):
        ...
        cursor = self.conn.execute(sqlalchemy.text('SHOW TABLES'))
        ...
        for tname in table_names:
            ...
            # 獲取所有column名稱
            c = self.conn.execute(sqlalchemy.text('Show columns from ' + tname + ';'))
            
            for cname in [i[0] for i in list(c)]:
        ...
        for tname in table_names:
            ...
            if 'date' in cnames:
                if tname == 'price':
                    ...
                    df = (pd.read_sql(sqlalchemy.text(s1), self.conn)
                          .append(pd.read_sql(sqlalchemy.text(s2), self.conn))
                          .append(pd.read_sql(sqlalchemy.text(s3), self.conn))
                    ...
                    self.dates[tname] = pd.read_sql(sqlalchemy.text(s), self.conn, parse_dates=['date'], index_col=['date']).sort_index()
        #print('Data: done')
